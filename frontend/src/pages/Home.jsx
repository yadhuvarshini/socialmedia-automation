...
export default function Home() {
    ...
    // Reload posts when selectedPlatform changes
    useEffect(() => {
        loadPosts(false);
    }, [selectedPlatform]);
    ...
    const handleDisconnectIntegration = async (integrationId) => {
        ...
        try {
            ...
            if (res.ok) {
                loadIntegrations();
                if (selectedPlatform === integrations.find(i => i.id === integrationId)?.platform) {
    ...
    const availablePlatforms = ['linkedin', 'facebook', 'twitter', 'instagram', 'reddit', 'threads'];

    return (
        <div className="home-v2">
            {/* Slim Profile & Platform Sidebar */}
            <aside className="sidebar-slim">
                <div className="sidebar-slim__brand" onClick={() => setSelectedPlatform(null)}>
                    <span className="sidebar-slim__logo">B</span>
                </div>

                <div className="sidebar-slim__platforms">
                    {availablePlatforms.map(platform => {
                        ...
                        return (
                            <button
                                key={platform}
                                className={`sidebar-slim__icon-btn ${selectedPlatform === platform ? 'active' : ''} ${!isConnected ? 'disconnected' : ''}`}
                                onClick={() => isConnected ? setSelectedPlatform(platform) : handleConnectIntegration(platform)}
                                title={isConnected ? `View ${platform}` : `Connect ${platform}`}
                            >
                                <div className="icon-wrapper" style={{ color: isConnected ? platformColors[platform] : '#718096' }}>
                ...
                <div className="sidebar-slim__footer">
                    <button className="sidebar-slim__user-btn" onClick={logout} title="Sign Out">
                        {user?.profile?.profilePicture ? (
                            <img src={user.profile.profilePicture} alt="" />
                        ) : (
                            <div className="avatar-placeholder">{(user?.email?.[0] || 'U').toUpperCase()}</div>
                        )}
                    </button>
                </div>
            </aside>
                ...
                {!selectedPlatform ? (
                    <div className="welcome-splash">
                        <div className="welcome-splash__card">
                            ...
                            <p>Select a platform from the sidebar to manage your accounts, schedule posts, and view history.</p>
                            <div className="welcome-splash__grid">
                                {availablePlatforms.map(p => (
                                    <div key={p} className="splash-icon" style={{ color: platformColors[p] }}>
                                        {platformIcons[p]}
                                    </div>
                                ))}
                            </div>
                    ...
                    <div className="contextual-view">
                        {/* Platform Hero Area */}
                        {integrations.filter(i => i.platform === selectedPlatform && i.isActive).map(integration => (
                            <header key={integration.id} className="platform-hero">
                                <div className="platform-hero__profile">
                                    <div className="platform-hero__avatar-container">
                                        <img
                                            src={integration.profile?.profilePicture || '/default-avatar.png'}
                                            alt=""
                                            className="platform-hero__avatar"
                                            onError={(e) => e.target.src = 'https://ui-avatars.com/api/?name=' + (integration.profile?.name || selectedPlatform)}
                                        />
                                        <div className="platform-hero__platform-badge" style={{ background: platformColors[selectedPlatform] }}>
                                            {platformIcons[selectedPlatform]}
                                        </div>
                                    </div>
                                    <div className="platform-hero__info">
                                        <h2>{integration.profile?.name || '-'}</h2>
                                        <p className="platform-hero__handle">@{selectedPlatform} account connected</p>
                                    </div>
                                </div>
                        ...
                        <div className="dashboard-grid">
                            {/* Dynamic Composer Section */}
                            <section className="dashboard-section composer-section">
                                <Suspense fallback={<div className="loading-shimmer">Preparing Composer...</div>}>
                                    <PostComposer
                            ...
                            <section className="dashboard-section posts-section">
                                ...
                                {loading && !loadingMore ? (
                                    <div className="loading-placeholder">Loading your activity...</div>
                                ) : (
                                    <>
